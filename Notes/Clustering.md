...
Where $\text{Expected RI}$ is specified by a `random model` measuring the `expected similarity` of all `pair-wise comparisons`. A $\textcolor{green}{\text{contingency table}}$ is used when calculating.
...
Then let $X$ be the clustering results, $Y$ be the ground truth labels, and then $MI$ reflects the $\textcolor{red}{\text{alignment between them}}$. For `Normalized Mutual Information`, with `1 as the perfect match`:$$\text{NMI}=\frac{2\cdot I(C,L)}{H(C)+H(L)}\in[0,1]$$
...
`Definition`: Clustering methods that represent each cluster by a $\textcolor{red}{\text{prototype (e.g., centroid, medoid)}}$ summarizing the group.

`Key Idea`: Assign data points to the nearest prototype, optimizing a $\textcolor{red}{\text{distance-based objective (e.g., sum of squares)}}$.
